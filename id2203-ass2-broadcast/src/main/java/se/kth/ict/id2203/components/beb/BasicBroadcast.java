	...
	Handler<BebBroadcast> handleBebBroadcast= new Handler<BebBroadcast>(){
		...
		public void handle(BebBroadcast event) {
			...
			for (Address address: init.getAllAddresses()){
				logger.info("Will beb broadcast message ");
				trigger(new Pp2pSend(address,new BebDataMessage(init.getSelfAddress(), event.getDeliverEvent())), pp2p);
			}
		}
	};
	
	Handler<BebDataMessage> handleBebDataMessage = new Handler<BebDataMessage>(){
		@Override
		public void handle(BebDataMessage event) {
			// TODO Auto-generated method stub
			logger.info("Receinved beb  data message ");
			trigger(event.getDeliver(),beb);
		}
